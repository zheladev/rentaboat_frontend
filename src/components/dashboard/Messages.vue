      ...
      <v-card flat outlined rounded="0" class="my-2">
        <v-list three-line class="py-0">
          <template v-for="(chat, index) in chats">
            <v-divider
              v-if="index > 0 && index < chats.length"
              :key="`${index}-divider`"
            ></v-divider>
            <v-list-item :key="index">
              <v-list-item-avatar>
                <v-img
                  :src="getFilePath(getChatParticipantImage(chat))"
                ></v-img>
              </v-list-item-avatar>
              <v-list-item-content>
                <v-list-item-title>
                ...
                <v-list-item-subtitle class="chat-last-message">
                  <v-icon
                    v-if="chat.lastMessage.user.id === user.id"
                    small
                    class="d-inline chat-sent-icon"
                    >check</v-icon
                  >
...
export default {
  ...
  methods: {
    ...
    getChatParticipantImage(chat) {
      return chat.creator.id === this.user.id
        ? chat.receiver.img
        : chat.creator.img;
    },
  },
